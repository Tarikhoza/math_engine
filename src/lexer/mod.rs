    ...
    fn read_next(&mut self) -> Token {
        ...
        if let Some(current_input) = self.input.get(self.pos..) {
            let (token_type, len): (TokenType, usize) = match current_input.to_string() {
                t if t.starts_with("\\rightarrow") => (TokenType::RightArrow, 11),
                t if t.starts_with("rightarrow") => (TokenType::RightArrow, 10),
                t if t.starts_with("\\leftarrow") => (TokenType::LeftArrow, 10),
                t if t.starts_with("leftarrow") => (TokenType::LeftArrow, 9),
                t if t.starts_with("\\mathrm") => (TokenType::FuncStart, 7),
                t if t.starts_with("mathrm") => (TokenType::FuncStart, 6),
                t if t.starts_with("\\lvert") => (TokenType::VertOpen, 6),
                t if t.starts_with("lvert") => (TokenType::VertOpen, 5),
                t if t.starts_with("\\rvert") => (TokenType::VertClose, 6),
                t if t.starts_with("rvert") => (TokenType::VertOpen, 5),
                t if t.starts_with("\\prod_") => (TokenType::ProdStart, 6),
                t if t.starts_with("prod_") => (TokenType::ProdStart, 5),
                t if t.starts_with("\\sum_") => (TokenType::SumStart, 5),
                t if t.starts_with("sum_") => (TokenType::SumStart, 4),
                t if t.starts_with("\\frac") => (TokenType::FracStart, 5),
                t if t.starts_with("frac") => (TokenType::FracStart, 4),
                t if t.starts_with("\\sqrt") => (TokenType::SqrtStart, 5),
                t if t.starts_with("sqrt") => (TokenType::SqrtStart, 4),
                t if t.starts_with("\\div") => (TokenType::Div, 4),
                t if t.starts_with("div") => (TokenType::Div, 3),
                t if t.starts_with("def") => (TokenType::DefKeyword, 3),
                t if t.starts_with("\\pm") => (TokenType::Div, 3),
                t if t.starts_with("pm") => (TokenType::Div, 2),
                t if t.starts_with("->") => (TokenType::RightArrow, 2),
                t if t.starts_with("<-") => (TokenType::LeftArrow, 2),
                t if t.starts_with("<=") => (TokenType::LessThanEqual, 2),
                t if t.starts_with(">=") => (TokenType::GreaterThanEqual, 2),
                t if t.starts_with("\\{") => (TokenType::BackSlashCurlyOpen, 2),
                t if t.starts_with("\\}") => (TokenType::BackSlashCurlyClose, 2),
                ...
                t if t.starts_with("_") => (TokenType::Underscore, 1),
