...
impl Parsable for Product {
    ...
    fn from_token_stream(token_stream: Vec<Token>) -> Result<(PolynomPart, usize), String> {
        env_info("parser", "Trying to parse token stream as prod".into());
        let mut len = 1;
        let iter: String;
        let start: Box<Math>;
        let end: Box<Math>;
        ...
        if let Some(token) = token_stream.first() {
            if token.token_type != TokenType::ProdStart {
                env_info("parser", "No prod token on begining...".into());
                return Err("No prod token on begining".into());
            }
        } else {
            env_info("parser", "No prod token on begining...".into());
            return Err("No prod token on begining".into());
        ...
        return Ok((
            PolynomPart::Math(Math::Product(Product {
                inner,
                start,
                end,
                iter,
